...
for (i in seq(nrow(param))) {
  ...
  signMat_wGeneName_diff <- keep_diffGenes(signMat = signMat_wGeneName, pvalMat = signMat_pval_wGeneName, th = 0.05)
  nbDiffGene <- nrow(signMat_wGeneName_diff)
  message("Number of differentially expressed genes : ", nbDiffGene, " / 978")
  
  saveSignMat(matrix = signMat_wGeneName, cellLine = cLine,
              output_dir = "output/analysis/lincs/coregulators_correlation_perCellLine_diffGenes",
              output_file = paste0("signMat_diffgenes", "_", cLine, "_", time_str, "_", date))
  
  signMat <- signMat_wGeneName_diff %>% select(-Name_GeneSymbol)
  ...
  heatmap_MUTCOF <- Heatmap(cor.pearson.mutcof, name = "Pearson correlation",
                            # Title
                            column_title = paste0(cLine, " | ", time, " | ", nbDiffGene, "/978 genes") ,
                            column_title_gp = gpar(fill = "black", col = "white", fontface = "bold", fontsize = "20"),
                            row_names_side = "left",
                            row_dend_side = "right",
                            column_names_side = "top",
  ...
  saveHeatmap(heatmap_obj = heatmap_MUTCOF,
              output_dir = "output/analysis/lincs/coregulators_correlation_perCellLine_diffGenes",
              output_file = paste0("heatmap_MUTCOF_diffGenes", cLine, "_", time_str, "_simplepearson_", date),
              format = "pdf",
              width = 15, height = 12)
}
