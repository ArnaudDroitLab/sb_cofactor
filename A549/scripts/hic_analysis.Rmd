...
Number of assessed genes: `r length(promoters(t_obj))`
...
metrics_fun = function(time_point) {
    data.frame(Genes=c(sum(mcols(promoters(t_obj))[[time_point]] == "Up"),
                       sum(mcols(promoters(t_obj))[[time_point]] == "Down"),
                       sum(mcols(promoters(t_obj))[[time_point]] == "Stable")),
               Direction=c("Up", "Down", "Stable"),
               Time=time_point)
}

```{r GR_binding_regulation_status, echo=FALSE}
as.data.frame(promoters(t_obj)) %>% 
    group_by(RegulationStatus) %>% 
    summarize(N=n(), 
              EarlyBindingN=sum(ConsensusGR>0), 
              EarlyBindingP=EarlyBindingN/N, 
              ContactBoundN=sum(ConsensusGR_ContactBound), 
              ContactBoundP=ContactBoundN/N,
              AnyBindingN=sum(ConsensusGR>0 | ConsensusGR_ContactBound | ConsensusGR_WindowBound),
...
```{r define_venn_plot_function, echo=FALSE}
    ...
    venn_plot <- function(de_value, main) {
        is_de = promoters(t_obj)$"DE-2h" %in% de_value
        
        direct_indices = is_de & promoters(t_obj)$ConsensusGR>0
        window_indices = is_de & promoters(t_obj)$ConsensusGR_WindowBound
        contact_indices = is_de & promoters(t_obj)$ConsensusGR_ContactBound
        
        gene_list = list(Direct=promoters(t_obj)$ensembl_gene_id[direct_indices],
                         Window=promoters(t_obj)$ensembl_gene_id[window_indices],
                         Contact=promoters(t_obj)$ensembl_gene_id[contact_indices])
...
metrics_fun = function(y, time_point) {
    de_genes = mcols(promoters(t_obj))[[time_point]] %in% direction_list[[y]]
    direct_binding = promoters(t_obj)$ConsensusGR > 0
    indirect_binding = promoters(t_obj)$ConsensusGR_WindowBound |
                       promoters(t_obj)$ConsensusGR_ContactBound 
