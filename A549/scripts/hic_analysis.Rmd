...
kable(as.data.frame(query_regions(t_obj)$Promoters) %>% 
    group_by(RegulationStatus) %>% 
    summarize(N=n(), 
              EarlyBindingN=sum(ConsensusGR>0), 
              EarlyBindingP=EarlyBindingN/N, 
              ContactN=sum(ConsensusGR_Contact), 
              ContactP=ContactN/N,
              AnyBindingN=sum(ConsensusGR>0 | ConsensusGR_Contact | ConsensusGR_Window),
...
kable(as.data.frame(query_regions(t_obj)$Promoters) %>% 
    ...
    summarize(N=n(), 
              ...
              EarlyBindingP=EarlyBindingN/N, 
              ContactN=sum(BRD4_DEX_Contact), 
              ContactP=ContactN/N,
              AnyBindingN=sum(BRD4_DEX>0 | BRD4_DEX_Contact | BRD4_DEX_Window),
              AnyBindingP=AnyBindingN/N))
```
