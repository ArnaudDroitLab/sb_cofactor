...
as.data.frame(hic_results$Promoters) %>% 
    group_by(RegulationStatus) %>% 
    summarize(N=n(), 
              EarlyBindingN=sum(EarlyBinding>0), 
              EarlyBindingP=EarlyBindingN/N, 
              ContactBoundN=sum(GR_ContactBound), 
              ContactBoundP=ContactBoundN/N,
              AnyBindingN=sum(EarlyBinding>0 | GR_ContactBound | GR_WindowBound),
              AnyBindingP=AnyBindingN/N)
              
as.data.frame(hic_results$Promoters) %>% 
    group_by(RegulationStatus) %>% 
    summarize(N=n(), 
              EarlyBindingN=sum(BRD4_DEX>0), 
              EarlyBindingP=EarlyBindingN/N, 
              ContactBoundN=sum(BRD4_DEX_ContactBound), 
              ContactBoundP=ContactBoundN/N,
              AnyBindingN=sum(BRD4_DEX>0 | BRD4_DEX_ContactBound | BRD4_DEX_WindowBound),
              AnyBindingP=AnyBindingN/N)              
```
...
```{r define_venn_plot_function, echo=FALSE}
    ...
    venn_plot <- function(de_value, main) {
        ...
        window_indices = is_de & hic_results$Promoters$GR_WindowBound
        contact_indices = is_de & hic_results$Promoters$GR_ContactBound
...
metrics_fun = function(y, time_point) {
    ...
    indirect_binding = hic_results$Promoters$GR_WindowBound |
                       hic_results$Promoters$GR_ContactBound 
