...
for (cofactor in cofactor_list) {
	metagene_obj_name <- file.path(output_dir, paste0(cofactor, "_UpRegulatedTSS_DownRegulatedTSS", "_metagene_obj.RData"))
	...
	mg_df$Direction <- as.factor(ifelse(grepl("UpRegulatedTSS", mg_df$group), "UpRegulatedTSS", "DownRegulatedTSS"))
	
	title <- paste0(cofactor, "_UpRegulatedTSS_DownRegulatedTSS", "_DEX_2hours")
	...
	ggplot(mg_df, aes(x=bin, y=value, ymin=qinf, ymax=qsup, group=Replicate)) +
		...
		facet_grid(Cofactor ~ Direction*Condition)
	
	output_filename <- file.path(output_dir, paste0(cofactor, "_UpRegulatedTSS_DownRegulatedTSS_w1000_metagene.pdf"))
	
	ggsave(output_filename, width = 14, height = 7, units = "in")
