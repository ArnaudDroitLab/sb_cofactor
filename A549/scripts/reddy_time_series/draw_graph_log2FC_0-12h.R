...
draw_time_course_pergroup_FC <- function(genes_group_list, counts_mat = FC_mat) {
  ...
  for (i in 1:length(genes_group_list)) {
    ...
    DEGs_group <- rep.int(group_n, nrow(df_mean))
    df_mean <- data.frame(df_mean, DEGs_group)
    colnames(df_mean) <- c("hours", "mean", "DEGs_group")
    df <- rbind(df, df_mean)
  }
  
  # Make the plot
  plot <- ggplot(df, aes(x = hours, y = mean)) +
    geom_point(aes(color = DEGs_group), size = 1) +
    #    theme(legend.position='none') +
    geom_line(data=df, aes(x=hours, y=mean, group=DEGs_group, color=DEGs_group)) +
    scale_x_continuous(name="Hours", labels=c(0, 0.5, 1, 2, 3, 4, 5, 6, 7, 8, 10, 12), breaks=c(0, 0.5, 1, 2, 3, 4, 5, 6, 7, 8, 10, 12)) +
