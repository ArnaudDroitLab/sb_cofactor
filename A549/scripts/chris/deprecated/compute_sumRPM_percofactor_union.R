...
for (cofactor in cofactors) {
  ...
  counts_reduced <- read.table(file = file.path(output_path, paste0("countTable_", cofactor, "_reduced.txt")),
                               header = TRUE)
  
  RPM_table <- getRPM_table(counts_reduced, cofactor)
  
  ### boxplot
  # boxplot_filename <- file.path(output_path, paste0("RPM_boxplot_", cofactor, ".png"))
  # png(boxplot_filename)
  # boxplot(RPM_table, outline = FALSE)
  # dev.off()
  
  sumRPM_reduced <- sumRPM(RPM_table, cofactor)
