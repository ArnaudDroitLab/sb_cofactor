...
symbol_prom_geneGainNB_ovGR <- gainNB_ovGR_annodf %>% filter(abs(distanceToTSS) <= 8000) %>% pull(SYMBOL) %>% unique
symbol_prom_geneGainNB_notovGR <- gainNB_notovGR_annodf %>% filter(abs(distanceToTSS) <= 8000) %>% pull(SYMBOL) %>% unique
symbol_prom_geneLossNB_ovGR <- lossNB_ovGR_annodf %>% filter(abs(distanceToTSS) <= 8000) %>% pull(SYMBOL) %>% unique
symbol_prom_geneLossNB_notovGR <- lossNB_notovGR_annodf %>% filter(abs(distanceToTSS) <= 8000) %>% pull(SYMBOL) %>% unique
...
POL2_diffbind_Myers <- load_diffbind_POLR2A_peaks_Myers()
POL2_UP_FDR_annodf <- POL2_diffbind_Myers[["POLR2A_UP_FDR"]] %>% annotatePeaks(output = "df"); print(nrow(POL2_UP_FDR_annodf)) # 242
gene_POL2_UP_FDR <- POL2_UP_FDR_annodf %>% pull(SYMBOL) %>% unique; print(length(gene_POL2_UP_FDR)) # 112

POL2_DOWN_FDR_annodf <- POL2_diffbind_Myers[["POLR2A_DOWN_FDR"]] %>% annotatePeaks(output = "df"); print(nrow(POL2_DOWN_FDR_annodf)) # 301
gene_POL2_DOWN_FDR <- POL2_DOWN_FDR_annodf %>% pull(SYMBOL) %>% unique; print(length(gene_POL2_DOWN_FDR)) # 168

POL2_UP_PVAL_annodf <- POL2_diffbind_Myers[["POLR2A_UP_PVAL"]] %>% annotatePeaks(output = "df"); print(nrow(POL2_UP_PVAL_annodf)) # 966
gene_POL2_UP_PVAL <- POL2_UP_PVAL_annodf %>% pull(SYMBOL) %>% unique; print(length(gene_POL2_UP_PVAL)) # 427

POL2_DOWN_PVAL_annodf <- POL2_diffbind_Myers[["POLR2A_DOWN_PVAL"]] %>% annotatePeaks(output = "df"); print(nrow(POL2_DOWN_PVAL_annodf)) # 1861
gene_POL2_DOWN_PVAL <- POL2_DOWN_PVAL_annodf %>% pull(SYMBOL) %>% unique; print(length(gene_POL2_DOWN_PVAL)) # 945

# control
all(gene_POL2_UP_FDR %in% gene_POL2_UP_PVAL)
all(gene_POL2_DOWN_FDR %in% gene_POL2_DOWN_PVAL)

# symbol
symbol_prom_geneGainNB_ovGR <- gainNB_ovGR_annodf %>% filter(abs(distanceToTSS) <= 8000) %>% pull(SYMBOL) %>% unique; print(length(symbol_prom_geneGainNB_ovGR)) # 63
symbol_prom_geneGainNB_notovGR <- gainNB_notovGR_annodf %>% filter(abs(distanceToTSS) <= 8000) %>% pull(SYMBOL) %>% unique; print(length(symbol_prom_geneGainNB_notovGR)) # 4
symbol_prom_geneLossNB_ovGR <- lossNB_ovGR_annodf %>% filter(abs(distanceToTSS) <= 8000) %>% pull(SYMBOL) %>% unique; print(length(symbol_prom_geneLossNB_ovGR)) # 551
symbol_prom_geneLossNB_notovGR <- lossNB_notovGR_annodf %>% filter(abs(distanceToTSS) <= 8000) %>% pull(SYMBOL) %>% unique; print(length(symbol_prom_geneLossNB_notovGR)) # 1593

### VS
# gain NB ov GR
sum(symbol_prom_geneGainNB_ovGR %in% gene_POL2_UP_FDR) # 8/63 # FIGURE
intersect(symbol_prom_geneGainNB_ovGR, gene_POL2_UP_FDR)

sum(symbol_prom_geneGainNB_ovGR %in% gene_POL2_UP_PVAL) # 17/63
intersect(symbol_prom_geneGainNB_ovGR, gene_POL2_UP_PVAL)

sum(symbol_prom_geneGainNB_ovGR %in% gene_POL2_DOWN_PVAL) # 2/63
intersect(symbol_prom_geneGainNB_ovGR, gene_POL2_DOWN_PVAL)

# loss NB ov GR
sum(symbol_prom_geneLossNB_ovGR %in% gene_POL2_DOWN_FDR) # 50/551
intersect(symbol_prom_geneLossNB_ovGR, gene_POL2_DOWN_FDR)

sum(symbol_prom_geneLossNB_ovGR %in% gene_POL2_DOWN_PVAL) # 148/551
intersect(symbol_prom_geneLossNB_ovGR, gene_POL2_DOWN_PVAL)

# loss NB ov GR
sum(symbol_prom_geneLossNB_notovGR %in% gene_POL2_DOWN_FDR) # 51/1593
intersect(symbol_prom_geneLossNB_notovGR, gene_POL2_DOWN_FDR)

sum(symbol_prom_geneLossNB_notovGR %in% gene_POL2_DOWN_PVAL) # 187/1593
intersect(symbol_prom_geneLossNB_notovGR, gene_POL2_DOWN_PVAL)
