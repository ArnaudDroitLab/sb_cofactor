...
countInBam_wce <- function(target, indexBam = FALSE) {
  ### List of bam
  report_bam <- make_report_bam(target_name = target, all_chip_bam)
  ...
  ### Save
  filename <- paste0("count_total_", target, "_wce")
  filename_rdata <- paste0(filename, ".RData")
  filename_txt <- paste0(filename, ".txt")
...
countInBam_background <- function(target, indexBam = FALSE) {
  report_bam <- make_report_bam(target_name = target, all_chip_bam)
  bamPath <- generate_bamPath_from_report(report_bam, "/home/chris/Bureau/sb_cofactor_hr/A549/input/ENCODE/A549/GRCh38/chip-seq/bam/")
  if (indexBam == TRUE) {indexBam(bamPath)}
  ...
  ### Save
  filename <- paste0("count_total_", target, "_background")
  filename_rdata <- paste0(filename, ".RData")
  filename_txt <- paste0(filename, ".txt")
...
targets2 <- c("SMC3", "CTCF", "RAD21", "FOSL2", "BCL3", "JUN", "JUNB", "HES2", "CEBPB")
for (protein in targets2) {
  message("#####\t", protein)
  countInBam(protein, indexBam = TRUE)
  countInBam_wce(protein, indexBam = TRUE)
  countInBam_background(protein, indexBam = FALSE)
  countInBam_gapbackground(protein, indexBam = FALSE)
}
