...
##########
build_intersect2 <- function (grl, keep.signal = FALSE) 
{
  all.regions = unlist(grl)
  overlap.matrix <- matrix(0, nrow = length(all.regions), ncol = length(grl))
  overlap.list = list()
  if (keep.signal) {
    signal.df <- data.frame(matrix(nrow = length(all.regions), 
                                   ncol = length(grl)))
  ...
  for (i in 1:length(grl)) {
    overlap.matrix[, i] <- GenomicRanges::countOverlaps(all.regions, 
                                                        grl[[i]], type = "any")
    overlap.list[[names(grl)[i]]] <- which(overlap.matrix[, 
                                                          i] != 0)
    if (keep.signal) {
      indices <- findOverlaps(all.regions, grl[[i]])
      signal.values.df <- data.frame(from = indices@from, 
                                     signal = grl[[i]]@elementMetadata@listData$signalValue[indices@to])
      if (nrow(signal.values.df) != 0 & sum(!is.na(signal.values.df$signal)) != 
          0) {
        signal.values.df <- aggregate(signal ~ from, 
                                      data = signal.values.df, FUN = mean, na.rm = TRUE)
  ...
  if (keep.signal) 
    mcols(all.regions) <- signal.df
  return(list(Regions = all.regions, Matrix = overlap.matrix, 
              List = overlap.list, Names = colnames(overlap.matrix), 
              Length = ncol(overlap.matrix)))
}
...
#
test <- build_intersect(NBC_CTRL)
M <- as.data.frame(test$Matrix); nrow(M) # 29929
NBC <- sum(M$NIPBL_CTRL != 0 & M$BRD4_CTRL != 0 & M$CDK9_CTRL != 0); print(NBC) # 5200
NB <- sum(M$NIPBL_CTRL != 0 & M$BRD4_CTRL != 0 & M$CDK9_CTRL == 0); print(NB) # 2439
NC <- sum(M$NIPBL_CTRL != 0 & M$BRD4_CTRL == 0 & M$CDK9_CTRL != 0); print(NC) # 104
BC <- sum(M$NIPBL_CTRL == 0 & M$BRD4_CTRL != 0 & M$CDK9_CTRL != 0); print(BC) # 3683
N <- sum(M$NIPBL_CTRL != 0 & M$BRD4_CTRL == 0 & M$CDK9_CTRL == 0); print(N) # 1080
B <- sum(M$NIPBL_CTRL == 0 & M$BRD4_CTRL != 0 & M$CDK9_CTRL == 0); print(B) # 16540
C <- sum(M$NIPBL_CTRL == 0 & M$BRD4_CTRL == 0 & M$CDK9_CTRL != 0); print(C) # 883
total <- NBC + NB + NC + BC + N + B + C; print(total) # 29929
